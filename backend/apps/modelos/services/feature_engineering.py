"""
Production feature engineering for olive ML models.

All lag/rolling/EMA/difference features are built from values shifted at least
one day into the past — no data leakage from current day into engineered features.
Current-day raw sensor columns remain unchanged.
"""
...
def _add_soil_profile(df: pd.DataFrame) -> pd.DataFrame:
    humidity_cols = [col for col, _ in HUMIDITY_DEPTHS if col in df.columns]
    if humidity_cols:
...
def add_features(
    ...
    """
    Return a copy of df augmented with engineered features for the requested variant.

    feature_cols should include both inputs and targets (autoregressive); production
    training passes targets + input_features so target lags are also available.
    For target_only variant, pass only [target] as feature_cols.
    """
    if variant not in VARIANTS:
        raise ValueError(f"Variante desconocida {variant!r}. Disponibles: {VARIANTS}")
